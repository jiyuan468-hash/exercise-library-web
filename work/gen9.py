...
export default function HomePage() {
  ...
  const handleCompare = (ex: Exercise) => {
    setCompareList(prev => {
      if (prev.includes(ex)) return prev.filter(e => e.id !== ex.id);
  ...
  return (
    <div style={s.container}>
      ...
      {compareList.length > 0 && <ComparePanel exercises={compareList} onClose={() => setCompareList([])} onRemove={id => setCompareList(prev => prev.filter(e => e.id !== id))} />}
    </div>
  );
}
