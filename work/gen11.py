...
# FavoritesPage.tsx
code = """import { useExercises } from '../hooks/useExercises';
...
with open(os.path.join(src, 'pages', 'FavoritesPage.tsx'), 'w', encoding='utf-8') as f:
    f.write(code)
print('FavoritesPage done')

# PlansPage.tsx
code2 = """import { useExercises } from '../hooks/useExercises';
import { useApp } from '../contexts/AppContext';
import { zh, en } from '../i18n';
import { WorkoutPlan, WorkoutExercise, Exercise } from '../types';
import { useState, useMemo } from 'react';
...
export default function PlansPage() {
  ...
  const favExercises = useMemo(() => exercises.filter(ex => favorites.includes(ex.id)), [exercises, favorites]);
  ...
  const handleRemoveExercise = (planId: string, idx: number) => {
    const plan = plans.find(p => p.id === planId);
    if (!plan) return;
    updatePlan({ ...plan, exercises: plan.exercises.filter((_, i) => i !== idx) });
  };
        ...
        plans.map(plan => (
          <div key={plan.id} style={s.planCard}>
            <div style={s.planHeader}>
              ...
              <span style={s.planCount}>{t.plans.exercises(plan.exercises.length)}</span>
              <button style={s.deleteBtn} onClick={() => deletePlan(plan.id)}>{t.plans.deletePlan}</button>
            </div>
            {plan.exercises.map((we, idx) => (
              <div key={idx} style={s.exerciseRow}>
                ...
                <div style={s.exControls}>
                  <select value={we.sets} onChange={e => handleUpdateExercise(plan.id, idx, 'sets', parseInt(e.target.value))} style={s.select}>
                    {[1,2,3,4,5,6].map(n => <option key={n} value={n}>{n} {t.plans.sets}</option>)}
                  </select>
                  <input value={we.reps} onChange={e => handleUpdateExercise(plan.id, idx, 'reps', e.target.value)} placeholder={t.plans.reps} style={s.inputSmall} />
            ...
            <div style={s.addRow}>
              <select value={newExerciseId} onChange={e => setNewExerciseId(e.target.value)} style={s.select}>
                ...
                {favExercises.map(ex => <option key={ex.id} value={ex.id}>{ex.name}</option>)}
              </select>
...
"""
with open(os.path.join(src, 'pages', 'PlansPage.tsx'), 'w', encoding='utf-8') as f:
    f.write(code2)
print('PlansPage done')
